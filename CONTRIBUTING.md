...
1. **Find the endpoint in the OpenAPI spec.** `src/shared/api/openapi.ts` is regenerated by `npm run gen:api-types` from the live API; the same script also rebuilds `src/shared/api/zod-schemas.ts` (runtime validation schemas). Both files are committed. Regen is manual and ungated — there is no CI drift job (it would couple this pipeline to the API's runtime availability), so the MR that changes `/api/v1` must run the generator and commit both files, and review is the only thing that catches a missed regen. Never hand-edit them.
