...
describe("resumeCampaignGroupScheduleTool", () => {
  ...
  it("returns schedule_paused=false", async () => {
    ...
    const r = await resumeCampaignGroupScheduleTool.handler({ group_id: GID }, makeToolContext({ api }));
  ...
  it("maps error", async () => {
    ...
    expect(
      (await resumeCampaignGroupScheduleTool.handler({ group_id: GID }, makeToolContext({ api }))).isErr()
    ).toBe(true);
  });
});
