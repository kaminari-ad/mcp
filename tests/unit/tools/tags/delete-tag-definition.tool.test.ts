...
describe("deleteTagDefinitionTool", () => {
  ...
  it("returns deleted=true", async () => {
    ...
    const r = await deleteTagDefinitionTool.handler({ slug: "x" }, makeToolContext({ api }));
    expect(r._unsafeUnwrap()).toEqual({ deleted: true });
  });
  it("maps error", async () => {
    const api = createFakeApiGateway();
    api.state.responses.deleteTagDefinition = err(makeApiError("forbidden", "system tag"));
    expect((await deleteTagDefinitionTool.handler({ slug: "x" }, makeToolContext({ api }))).isErr()).toBe(true);
  });
});
