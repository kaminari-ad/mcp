...
describe("deletePolicySetTool", () => {
  ...
  it("returns deleted=true and forwards the policy-set id", async () => {
    ...
    expect(api.state.calls[0]).toEqual({ method: "deletePolicySet", id: PID });
  });
  it("maps error", async () => {
    const api = createFakeApiGateway();
    api.state.responses.deletePolicySet = err(makeApiError("forbidden", "x"));
    expect(
      (await deletePolicySetTool.handler({ policy_set_id: PID }, makeToolContext({ api }))).isErr()
    ).toBe(true);
  });
});
