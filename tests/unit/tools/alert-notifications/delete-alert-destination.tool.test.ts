...
describe("deleteAlertDestinationTool", () => {
  ...
  it("returns deleted=true", async () => {
    ...
    const r = await deleteAlertDestinationTool.handler({ destination_id: DID }, makeToolContext({ api }));
    expect(r._unsafeUnwrap()).toEqual({ deleted: true });
  });
  it("maps error", async () => {
    const api = createFakeApiGateway();
    api.state.responses.deleteAlertDestination = err(makeApiError("not-found", "x"));
    expect((await deleteAlertDestinationTool.handler({ destination_id: DID }, makeToolContext({ api }))).isErr()).toBe(true);
  });
});
