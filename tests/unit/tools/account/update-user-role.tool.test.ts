...
describe("updateUserRoleTool", () => {
  it("name + uuid + idempotent annotation", () => {
    ...
    expect(() => updateUserRoleTool.inputSchema.parse({ user_id: "x", role: "y" })).toThrow();
  });
  it("forwards role change", async () => {
    const api = createFakeApiGateway();
    await updateUserRoleTool.handler({ user_id: UID, role: "viewer" }, makeToolContext({ api }));
    const call = api.state.calls[0];
    if (call?.method !== "updateUserRole") throw new Error("wrong");
    expect(call.userId).toBe(UID);
    expect(call.body.role).toBe("viewer");
  });
  it("maps error", async () => {
    const api = createFakeApiGateway();
    api.state.responses.updateUserRole = err(makeApiError("not-found", "x"));
    expect(
      (await updateUserRoleTool.handler({ user_id: UID, role: "admin" }, makeToolContext({ api }))).isErr()
    ).toBe(true);
  });
});
