...
import { parseRun, parseRunPage } from "../../../../../src/infrastructure/api/parsers/parse-run.js";
...
describe("parseRunPage", () => {
  it("Ok valid", () => {
    expect(parseRunPage({ items: [VALID], total: 1, page: 1, limit: 50 }).isOk()).toBe(true);
  });

  it("rejects non-object", () => {
    expect(parseRunPage("x").isErr()).toBe(true);
  });

  it("rejects bad envelope", () => {
    expect(parseRunPage({ items: "x", total: 1, page: 1, limit: 50 }).isErr()).toBe(true);
  });

  it("rejects bad item", () => {
    expect(parseRunPage({ items: [{}], total: 1, page: 1, limit: 50 }).isErr()).toBe(true);
  });
});
