...
| `stateless_no_session.test.ts`  | The transport is stateless: `initialize` issues no `Mcp-Session-Id`, and `tools/list` succeeds with no session id (no 400). Any replica serves any request. |
